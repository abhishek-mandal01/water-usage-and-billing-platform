...
// Using a beautiful primary blue for the light theme: rgb(37, 99, 235)
...
export const MagicCardGrid = ({ children, className = '' }) => {
  const gridRef = useRef(null);
  const isMobile = useMobileDetection();
  
  return (
    <div className={`bento-section ${className}`} ref={gridRef} style={{ position: 'relative' }}>
      <GlobalSpotlight
        gridRef={gridRef}
...
export const MagicCard = ({ children, className = '', style, disableAnimations = false }) => {
  const isMobile = useMobileDetection();
  
  return (
    <ParticleCard
      className={`magic-bento-card--border-glow ${className}`}
      style={style}
